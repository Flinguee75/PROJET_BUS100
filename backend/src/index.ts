...
if (process.env.FUNCTIONS_EMULATOR === 'true') {
  const websocketPort = parseInt(process.env.WEBSOCKET_PORT ?? '8181', 10);
  websocketManager.initialize(websocketPort);
  console.log(`🚀 Mode Emulator: WebSocket activé sur port ${websocketPort}`);
}
...
export { cleanupArrivedBuses } from './triggers/arrivedBusCleanup.trigger';
